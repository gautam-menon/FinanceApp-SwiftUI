...
//    }
    ...
    func fetchData(myId: String, sortParam: String, desc: Bool, onlyMyTransactions: Bool) async {
        do {
            if onlyMyTransactions {
              let querySnapshot = try await db.collection(transactionCollection).whereField("name", isNotEqualTo: myId).getDocuments()
                self.addData(documents: querySnapshot.documents, myId: myId)
            } else {
                let querySnapshot = try await db.collection(transactionCollection).order(by:sortParam, descending: desc).getDocuments()
